...
const createDomainBlock = () => ({
  ...
  subDomain: '',
  levels: [createLevelBlock()],
  flatCompetencies: [createComp()],
});
const createRscRoleItem = () => ({
  id: `${Date.now()}-${Math.random()}`,
  role: '',
  domains: [createDomainBlock()],
});

const isDomainBlockInvalid = (d) => {
  if (!d.domain) {
    return true;
  }
    ...
    <div className="framework-builder__section-card framework-builder__section-card--form">
      <div className="framework-builder__role-head">
        ...
        <button
          ...
          onClick={() => mutate(prev => [...prev, createRscRoleItem()])}
        >
      ...
      <div className="framework-builder__org-type-list">
        {items.map((roleItem) => {
          ...
          const anyDomainInvalidInRole = roleItem.domains.some(isDomainBlockInvalid);
          ...
          return (
            <div key={roleItem.id} className="framework-builder__org-type-card">
              ...
              <div className="framework-builder__rsc-role-inner">
                <div className="framework-builder__rsc-type-line">
                  <span className="framework-builder__proficiency-label">{labels.competencyTypeLabel}</span>
                  <span className="framework-builder__rsc-type-value">{labels.competencyTypeValue}</span>
                </div>
                ...
                <div className="framework-builder__org-domain-list">
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                ...
                                updateDomain(checked
                                  ? { requireProficiency: true, levels: [createLevelBlock()] }
                                  : { requireProficiency: false, flatCompetencies: [createComp()] });
                              }}
                              disabled={!canEdit}
                            />
                        ...
                        <div className="framework-builder__org-level-list framework-builder__rsc-nested">
                          {domainItem.requireProficiency ? (
                            <>
                                  ...
                                  <div key={levelItem.id} className="framework-builder__org-level-card">
                                                ...
                                                domains: r.domains.map(d => (
                                                  d.id === domainItem.id
                                                    ? {
                                                      ...
                                                      levels: d.levels.map(l => (
                                                        l.id === levelItem.id
                                                          ? { ...l, competencies: [...l.competencies, createComp()] }
                                                          : l
                                                      )),
                                                    }
                                                    : d
                                    ...
                                    <div className="framework-builder__org-competency-list">
                                      {levelItem.competencies.map((compItem, compIndex) => {
                                        const canDeleteComp = levelItem.competencies.length > 1;
                                        return (
                                          <div key={compItem.id} className="framework-builder__org-competency-row">
                                            ...
                                            <button
                                              type="button"
                                              className="competency-framework-page__icon-button competency-framework-page__icon-button--danger"
                                              disabled={!canEdit || !canDeleteComp || isDeleting}
                                              onClick={() => removeWithGuard(
                                                canDeleteComp,
                                                () => mutate(prev => prev.map(r => (
                                                  r.id === roleItem.id
                                                          ...
                                                          ? {
                                                            ...
                                                            levels: d.levels.map(l => (
                                                              l.id === levelItem.id
                                                                ? {
                                                                  ...l,
                                                                  competencies: l.competencies.filter(
                                                                    c => c.id !== compItem.id,
                                                                  ),
                                                                }
                                                                : l
                              ...
                              <button
                                ...
                                disabled={!canEdit || domainItem.levels.some(level => (
                                  !level.proficiencyLevel || level.competencies.some(comp => !comp.text.trim())
                                ))}
                                onClick={() => mutate(prev => prev.map(r => (
                                  r.id === roleItem.id
                                    ? {
                                      ...
                                      domains: r.domains.map(d => (
                                        d.id === domainItem.id
                                          ? { ...d, levels: [...d.levels, createLevelBlock()] }
                                          : d
                                      )),
                                    }
                                    : r
                            ...
                            <div className="framework-builder__org-level-card framework-builder__rsc-flat-card">
                              <div className="framework-builder__org-row-head">
                                <label className="framework-builder__proficiency-label">{labels.flatCompetenciesLabel}</label>
                                <button
                                  type="button"
                                  className="competency-framework-page__outline-button framework-builder__org-small-add"
                                  disabled={!canEdit || domainItem.flatCompetencies.some(comp => !comp.text.trim())}
                                  onClick={() => mutate(prev => prev.map(r => (
                                    r.id === roleItem.id
                                      ? {
                                        ...r,
                                        domains: r.domains.map(d => (
                                          d.id === domainItem.id
                                            ? { ...d, flatCompetencies: [...d.flatCompetencies, createComp()] }
                                            : d
                                        )),
                                      }
                                      : r
                              ...
                              <div className="framework-builder__org-competency-list">
                                {domainItem.flatCompetencies.map((compItem, compIndex) => {
                                  const canDeleteComp = domainItem.flatCompetencies.length > 1;
                                  return (
                                    <div key={compItem.id} className="framework-builder__org-competency-row">
                                      <span className="framework-builder__org-competency-index">{`${compIndex + 1}.`}</span>
                                      <input
                                            ...
                                            ? {
                                              ...
                                              domains: r.domains.map(d => (
                                                d.id === domainItem.id
                                                  ? {
                                                    ...d,
                                                    flatCompetencies: d.flatCompetencies.map(c => (
                                                      c.id === compItem.id ? { ...c, text: event.target.value } : c
                                                    )),
                                                  }
                                                  : d
                                              )),
                                              ...
                                              ? {
                                                ...
                                                domains: r.domains.map(d => (
                                                  d.id === domainItem.id
                                                    ? {
                                                      ...d,
                                                      flatCompetencies: d.flatCompetencies.filter(
                                                        c => c.id !== compItem.id,
                                                      ),
                                                    }
                                                    : d
                  ...
                  <button
                    ...
                    onClick={() => mutate(prev => prev.map(r => (
                      r.id === roleItem.id
                        ? { ...r, domains: [...r.domains, createDomainBlock()] }
                        : r
                    )))}
                  >
                    <FontAwesomeIcon icon={faPlus} />
...
export { createRscRoleItem };
export default RoleSpecificCompetenciesTab;
